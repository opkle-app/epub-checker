...
/**
 * AbstractNode is the vanilla-TS DOM builder the whole renderer is built on
 * (no React/Vue — see docs/RENDERER_GUIDE.md). The framework is used mostly
 * through its static helpers:
 *
 *   const { createDom } = AbstractNode;
 *   createDom({ mode: "div", class: [...], children: [...] });
 *
 * createDom()/createSvg() are the two entry points that turn a DomCommand
 * object (see publicDictionary.ts) into a real HTMLElement/SVG tree,
 * recursing through `child`/`children`. Everything above them in this file
 * (colorExtended, svgMaker, media query helpers) is shared framework data —
 * a broader design-system palette/icon set than any single app necessarily
 * uses, kept here so it stays interchangeable across the AbstractNode-based
 * apps this framework was built for.
 */
class AbstractNode {
  ...
  public static svgMaker: Record<string, (...args: any[]) => string> = {
    returnLoading: function (
      color: string = AbstractNode.colorExtended.darkShadow,
      color2: string = AbstractNode.colorExtended.black,
    ): string {
    ...
    horizontalArrow: function (
      width: number,
      height: number,
      color: string = AbstractNode.colorExtended.blue,
      styleInjection: Record<string, string | number> | null = null,
    ): string {
      if (height === 0) {
  ...
  // Responsive breakpoint helpers used to pick size-dependent values (see
  // mediaNumber/mediaString) against `mediaCondition`. Falls back to the
  // parent frame's width when running inside an iframe/webview.
  ...
  // Dispatches to createSvg() for `mode: "svg"` commands, createDom() otherwise.
  ...
  /**
   * Turns one DomCommand into a real HTMLElement, recursively. Rough phases:
   *   1. Read command fields (mode/class/id/attribute/style/text/event) with
   *      safe defaults for anything omitted.
   *   2. Create the element (`mode` is the tag name; "abstract-div" is the
   *      default when omitted) and apply class/id/attribute/style/events.
   *   3. Resolve inline text formatting: `text` can contain small markup
   *      tokens (e.g. `<b%...<b%`) that get replaced with style-carrying
   *      tags built from the matching bold/italic/under/strike/special/code/
   *      reference/anchor DomStyle options.
   *   4. Recurse into `child`/`children` (each built via createDom/createSvg
   *      again) and append them in order; `next`/`previous` insert siblings
   *      relative to `mother` instead of appending as a child.
   *   5. If `mother` was provided, attach the finished element there;
   *      otherwise return it for the caller to place.
   */
  public static createDom = (domCommandObject: DomCommand): HTMLElement => {
    ...
    if (domCommandObject.child !== undefined) {
      if (
        typeof domCommandObject.child === "object" &&
        domCommandObject.child !== null &&
        !Array.isArray(domCommandObject.child)
      ) {
    ...
    if (domCommandObject.next !== undefined) {
      if (
        typeof domCommandObject.next === "object" &&
        domCommandObject.next !== null &&
    ...
    if (domCommandObject.previous !== undefined) {
      if (
        typeof domCommandObject.previous === "object" &&
        domCommandObject.previous !== null &&
        !Array.isArray(domCommandObject.previous)
      ) {
    ...
    if (domCommandObject.text !== undefined) {
      if (
        (typeof domCommandObject.text === "string" || Array.isArray(domCommandObject.text)) &&
          ...
          if (
            ...
            for (let b in domCommandObject.bold) {
              if (b === "color") {
                ...
                boldObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
          ...
          if (
            ...
            for (let b in domCommandObject.under) {
              if (b === "fontSizeWeight") {
                ...
                underObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
                  ? b.replace(/([a-z]+)([A-Z][a-z]+)/g, (match, p1, p2) => p1 + "-" + p2.toLowerCase())
                  : b;
                underObject += ":";
          ...
          if (
            ...
            for (let b in domCommandObject.special) {
              if (b === "fontSizeWeight") {
                ...
                specialObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
                  ? b.replace(/([a-z]+)([A-Z][a-z]+)/g, (match, p1, p2) => p1 + "-" + p2.toLowerCase())
                  : b;
                specialObject += ":";
          ...
          if (
            ...
            for (let b in domCommandObject.code) {
              if (b === "fontSizeWeight") {
                ...
                codeObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
                  ? b.replace(/([a-z]+)([A-Z][a-z]+)/g, (match, p1, p2) => p1 + "-" + p2.toLowerCase())
                  : b;
                codeObject += ":";
          ...
          if (
            ...
            for (let b in domCommandObject.italic) {
              if (b === "fontSizeWeight") {
                ...
                italicObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
                  ? b.replace(/([a-z]+)([A-Z][a-z]+)/g, (match, p1, p2) => p1 + "-" + p2.toLowerCase())
                  : b;
                italicObject += ":";
          ...
          if (
            ...
            for (let b in domCommandObject.reference) {
              if (b === "fontSizeWeight") {
                ...
                referenceObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
                  ? b.replace(/([a-z]+)([A-Z][a-z]+)/g, (match, p1, p2) => p1 + "-" + p2.toLowerCase())
                  : b;
                referenceObject += ":";
          ...
          if (
            ...
            for (let b in domCommandObject.anchor) {
              if (b === "fontSizeWeight") {
                ...
                anchorObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
                  ? b.replace(/([a-z]+)([A-Z][a-z]+)/g, (match, p1, p2) => p1 + "-" + p2.toLowerCase())
                  : b;
                anchorObject += ":";
          ...
          if (
            ...
            for (let b in domCommandObject.strike) {
              if (b === "fontSizeWeight") {
                ...
                strikeObject += /([a-z]+)([A-Z][a-z]+)/g.test(b)
                  ? b.replace(/([a-z]+)([A-Z][a-z]+)/g, (match, p1, p2) => p1 + "-" + p2.toLowerCase())
                  : b;
                strikeObject += ":";
    ...
    for (let key in targetStyle) {
      if (typeof targetStyle[key] === "string" && /;+$/gi.test(targetStyle[key])) {
    ...
    if (mother !== null && typeof mother.appendChild === "function") {
      if (
        ...
        (typeof domCommandObject.position === "number" && domCommandObject.position >= 0)
      ) {
        try {
          const motherChildren = [...mother.children];
          if (motherChildren.length > 0) {
            if (typeof domCommandObject.position === "number" && domCommandObject.position >= 0) {
              if (
                domCommandObject.position < motherChildren.length &&
                motherChildren[domCommandObject.position] !== undefined
              ) {
    ...
    if (mode === "video") {
      ...
      if (domCommandObject.attribute !== undefined) {
        if (typeof domCommandObject.attribute === "object" && domCommandObject.attribute !== null) {
          if (
            typeof domCommandObject.attribute.autoplay === "string" &&
            domCommandObject.attribute.autoplay !== "false"
          ) {
          ...
          if (
            (typeof domCommandObject.attribute.controls === "string" &&
              domCommandObject.attribute.controls !== "false") ||
  ...
  // SVG counterpart to createDom(): same DomCommand shape, but builds/returns
  // a SvgDom-wrapped SVGElement (see publicDictionary.ts) since SVG elements
  // need the XML namespace and don't share HTMLElement's full API.
  ...
  // Small CSS `calc()` string builders (withOut/cssCalc/pixelUnit/eaUnit) so
  // layout code can express "100% minus this many pixels" etc. without
  // hand-concatenating calc() strings everywhere.
  ...
  // Throttle (not debounce): the first call fires immediately, then further
  // calls are ignored until `ms` has passed.
